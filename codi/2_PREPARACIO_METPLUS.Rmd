...
llista_outcomes <- extreure.variables("outcomes1",conductor_variables)
...
dt_temp<-dt_temp %>% filter(categoria=="grupISGLT2") %>% 
...
forest_ISGLT2vsIDPP4<-forest.plot.met(dt_temp,mean="estimate",lower="Linf",upper="Lsup", color=TRUE)+
  ggtitle("DPP4i vs. SGLT-2i") +theme(plot.title = element_text(hjust = 0.5)) 


# 2. SU vs IDPP4 (ref)
dt_temp<-taula_estimacions_GRUP
dt_temp<-dt_temp %>% filter(categoria=="grupSU") %>% 
  transmute(datos,type,outcome,estimate=Std_Coefficient,Linf=CIStd_low,Lsup=CIStd_high)
...
# Plotejar ISGLT2 vs IDPP4
forest_SUvsIDPP4<-forest.plot.met(dt_temp,mean="estimate",lower="Linf",upper="Lsup", color=TRUE)+
  ggtitle("DPP4i vs. SU") + theme(plot.title = element_text(hjust = 0.50)) 


# 3. SU vs ISGLT2
dt_temp<-taula_estimacions_GRUP_ref_ISGLT2
# Preparar taula filtrant per grup i seleccionar camps
dt_temp<-dt_temp %>% filter(categoria=="grup2SU") %>% 
  transmute(datos,type,outcome,estimate=Std_Coefficient,Linf=CIStd_low,Lsup=CIStd_high)
...
# Plotejar SU vs ISGLT2
forest_SUvsISGLT2<- forest.plot.met(dt_temp,mean="estimate",lower="Linf",upper="Lsup", color=TRUE)+ ggtitle("SGLT-2i vs. SU") +theme(plot.title = element_text(hjust = 0.5)) 
