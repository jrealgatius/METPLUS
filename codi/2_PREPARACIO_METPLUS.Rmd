...
# CAnviar directori d'entrada / sortida

if (params$mostra=="test") {
  dir_dades<-"dades/preparades_test" 
  dir_resultats<-"resultats/test" 
  num_imputacions<-2 # Normalment >=10
  num_iteracions<-2 # Normalment 5

  } else {
    dir_dades<-"dades/preparades" 
...
# Canviar nom de l'arxiu en funció OT / ITT
...
# Lectura dades
  dades<-readRDS(here::here(dir_dades,arxiudades))
  # dt_historic_fd<-readRDS(here::here(dir_dades,"historic_fd.rds"))
  # dt_historic_fd_sgaps<-readRDS(here::here(dir_dades,"historic_fd_sgaps.rds"))
...
dadesmatching<-etiquetar_valors(dadesmatching,conductor_variables, "value_labels")
dadesmatching<-etiquetar(dadesmatching,conductor_variables)
...
# Genero Plot pre-post
# Canviar etiqutess 
b3<-cobalt::bal.tab(formulaPS,data=dades_sub1)
v1<-cobalt::var.names(b3,type = "vec",minimal = TRUE)  # Extreure variables
v1<-etiquetar_vector(v1,taulavariables = conductor_variables) # Etiquetar i posar variables
plot_ISGLT2_IDPP41<-cobalt::love.plot(m.out,var.names = v1,thresholds = c(m=.1), 
                  sample.names = c("Unmatched", "Matched"),limits=c(-0.5,2),position = "top",title = "SGLT-2i vs DPP-4i group")
...
plot_ISGLT2_SU<-cobalt::love.plot(m.out,var.names = v1,thresholds = c(m=.1), 
                sample.names = c("Unmatched", "Matched"),limits=c(-0.5,2),position = "top",title = "SGLT-2i vs SU group")
...
dadesmatching<-dadesmatching %>% left_join(select(dades_sub1,idp,ps2),by="idp")
...
save.image(here::here(dir_resultats,"temp.Rdata"))
...
# load(here::here(dir_resultats,"temp.Rdata"))
...
save.image(here::here(dir_resultats,"temp2.Rdata"))
...
# Actualitzar imatge / Carregar imatge

# load(here::here(dir_resultats,"temp2.Rdata"))
...
> Recodes i canvis d'escala de HBA1C post imputation

```{r}

# Calcul de Variable nueva IFCC HbA1c (nmmol/mol) = (10.93*HB1AC)-23.50
dades<-
  dades %>% mutate(HBA1C_IFCC=(HBA1C.valor*10.93)-23.50,
                 HBA1C_IFCC6m=(HBA1C.valor6m*10.93)-23.50,
                 HBA1C_IFCC12m=(HBA1C.valor12m*10.93)-23.50,
                 HBA1C_IFCC24m=(HBA1C.valor24m*10.93)-23.50,
                 HBA1C_IFCC324m=(HBA1C.valor324m*10.93)-23.50)
...
dades<-dades %>% mutate (
  ...
  HBA1C_IFCC.dif324m=HBA1C_IFCC-HBA1C_IFCC324m,
  HBA1C.valor324m_7cat=if_else(HBA1C.valor324m<7,1,0)) 


# 9.1. Calcular reduccions de PAS COLHDL.valor COLLDL.valor COLTOT.valor TG.valor
dades<-dades %>% mutate (
  PAS.dif324m=PAS.valor-PAS.valor324m,
  PAD.dif324m=PAD.valor-PAD.valor324m,
...
save.image(here::here(dir_resultats,"temp3.Rdata"))
...
# load(here::here(dir_resultats,"temp3.Rdata"))
...
## Estimacions d'efectes de tractaments 2 a 2 (Coeficients)


```{r, message=FALSE,warnings=FALSE}

level_conf<-params$nivell_conf
...
taula_estimacions_GRUP<-llista_outcomes %>% purrr::map_df(
  ~extreure_resum_outcomes_imputation(dades,outcome=.x,grups="grup",v.ajust=vector_ajust,level_conf = level_conf)) %>% 
  filter(categoria!="(Intercept)") %>% select(-mean)

taula_estimacions_GRUP_ref_ISGLT2<-llista_outcomes %>% purrr::map_df(
  ~extreure_resum_outcomes_imputation(dades,outcome=.x,grups="grup2",v.ajust=vector_ajust,level_conf = level_conf)) %>% 
  filter(categoria!="(Intercept)") %>% select(-mean)


```

## Estimacions 1 vs (2+3)  2 vs (1+3)

```{r}

# Respecte global
taula_estimacions_IDPP4<-llista_outcomes %>% purrr::map_df(
  ~extreure_resum_outcomes_imputation(dades,outcome=.x,grups="grup_IDPP4",v.ajust=vector_ajust,level_conf = level_conf)) %>% 
  filter(categoria!="(Intercept)") %>% select(-mean)

taula_estimacions_SU<-llista_outcomes %>% purrr::map_df(
  ~extreure_resum_outcomes_imputation(dades,outcome=.x,grups="grup_SU",v.ajust=vector_ajust,level_conf = level_conf)) %>% 
  filter(categoria!="(Intercept)") %>% select(-mean)

taula_estimacions_ISGLT2<-llista_outcomes %>% purrr::map_df(
  ~extreure_resum_outcomes_imputation(dades,outcome=.x,grups="grup_ISGLT2",v.ajust=vector_ajust,level_conf = level_conf)) %>% 
  filter(categoria!="(Intercept)") %>% select(-mean)
...
## Outcomes secundarios 2 taula 7 
...
# Remove alguns outcomnes
llista_outcomes<-llista_outcomes[!llista_outcomes %in% c("tempsTX.GRUP","temps.STOP.FD")]

# elimino de la llista outcomes alguns
...
# Salvar temporal
save.image(here::here(dir_resultats,"temp4.Rdata"))
...
# load(here::here(dir_resultats,"temp4.Rdata"))
...
# Panels plot d'efectes principals (3)

## Outcomes principals (3 ForestPlot) 2 a 2

```{r, warning=FALSE,message=FALSE}
# Outcomes principals calcular effect size (Standardized diference)
# Separar outcomes primaries de secundaries
...
forest_ISGLT2vsIDPP4<-forest_MET(taula_estimacions_GRUP,"grupISGLT2",titul ="DPP4i vs. SGLT-2i",color=TRUE)
# 2. SU vs IDPP4 (ref)
forest_SUvsIDPP4<-forest_MET(taula_estimacions_GRUP,"grupSU",titul = "DPP4i vs. SU", color=TRUE)

# 3. SU vs ISGLT2 (ref)
forest_SUvsISGLT2<-forest_MET(taula_estimacions_GRUP_ref_ISGLT2,"grup2SU",titul="SGLT-2i vs. SU", color=TRUE)
...
# Plotejar ISGLT2 vs IDPP4
forest_ISGLT2vsIDPP4_1<-forest_MET(taula_estimacions_GRUP,"grupISGLT2",titul ="DPP4i vs. SGLT-2i",color=TRUE,llista_out1)
# 2. SU vs IDPP4 (ref)
forest_SUvsIDPP4_1<-forest_MET(taula_estimacions_GRUP,"grupSU",titul = "DPP4i vs. SU", color=TRUE,llista_out1)
# 3. SU vs ISGLT2 (ref)
forest_SUvsISGLT2_1<-forest_MET(taula_estimacions_GRUP_ref_ISGLT2,"grup2SU",titul="SGLT-2i vs. SU", color=TRUE,llista_out1)

# Secundaries 

# Plotejar ISGLT2 vs IDPP4
forest_ISGLT2vsIDPP4_2<-forest_MET(taula_estimacions_GRUP,"grupISGLT2",titul ="DPP4i vs. SGLT-2i",color=TRUE,llista_out2)
# 2. SU vs IDPP4 (ref)
forest_SUvsIDPP4_2<-forest_MET(taula_estimacions_GRUP,"grupSU",titul = "DPP4i vs. SU", color=TRUE,llista_out2)
# 3. SU vs ISGLT2 (ref)
forest_SUvsISGLT2_2<-forest_MET(taula_estimacions_GRUP_ref_ISGLT2,"grup2SU",titul="SGLT-2i vs. SU", color=TRUE,llista_out2)
...
# 
# 
# forest_plot_Panel+labs(caption="Magnitude of effect size by Cohen's f: Small: <0.10; Medium: 0.25; Large: >0.4")
...
save(dades,dades_completes,flow_global,flow_global2,taula1,taula1.2,taulaPS, 
     ...
     file=here::here(dir_resultats,output))
