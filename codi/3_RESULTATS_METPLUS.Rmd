...
formula.LOGIT(x="grup", y=outcome, taulavariables = conductor_variables) %>% 
  extreure_OR(dades=dades) %>% tail(2) %>% 
...
formula.LOGIT(x="grup_ajustat", y=outcome, taulavariables = conductor_variables) %>% 
  extreure_OR(dades=dades) %>% tail(2) %>% 
  kable(caption = paste0("Odds ratio reducción de HBA1C ajustados por: ",paste0(extreure.variables("grup_ajustat",conductor_variables),collapse = ", "),digits = 3)) %>%   
  kableExtra::kable_styling()
